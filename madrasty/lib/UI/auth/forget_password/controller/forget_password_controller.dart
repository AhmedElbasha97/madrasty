...
class ForgetPasswordController extends GetxController {
  ...
  sinningIn(context) async {
    ...
    if(phoneState&&chosenUserType !=( chooseUserKey.tr)){

      AuthModel? data = await AuthServices.forgettingPassword(

          "${selectedCountryCode?.code ?? ""}${phoneController.text}", chosenUserType);
      if(data?.status == "true"){
        ...
        await Get.find<StorageService>().saveCheckerForgettingPassword(true);
        await Get.to(()=>const OtpScreen(comingFromSignUp: false,comingFromForgetPassword: true,));
      }else{
        isLoading = false;
        update();
  ...
  @override
  String toString() {
    return 'LoginController{ _email: ${phoneController.value},}';
  }
}