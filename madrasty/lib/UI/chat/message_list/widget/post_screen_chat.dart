// ignore_for_file: prefer_is_empty, use_build_context_synchronously, depend_on_referenced_packages, deprecated_member_use
...
import 'package:madrasty/UI/chat/message_list/widget/post_loading_screen_chat.dart';
...
class PostScreenChat extends StatefulWidget {
  const PostScreenChat({super.key, required this.postId});
  final String postId;

  @override
  State<PostScreenChat> createState() => _PostScreenChatState();
}

class _PostScreenChatState extends State<PostScreenChat> {
  Post? postData;
  late  VideoPlayerController videoPlayerController;
  late TextEditingController commentTextController;
  List<ParentShareModel>? parentSharModel = [];

  late List<StudentShareModel>? peopleSareList;
  TeacherInfoModel? teacherData;
  PersonInfoModel? personData;
 ...
 bool isLoadingPost = true;
  ...
  bool showingAnotherComments = false;
  @override
  void initState() {
    ...
    if((postData?.type == "VIDEOS")||(postData?.type == "VID")){
      videoPlayerController =
          VideoPlayerController.networkUrl(Uri.parse("${Services.baseUrl}${postData?.images?[0]??""}"))
        ..initialize().then((_) {
        ...
        builder: (BuildContext context) {
          return SingleChildScrollView(
            child: Padding(
              ...
              child: Column(
                children: parentSharModel!.map<Widget>((e) {
                  return SizedBox(
                    width: Get.width,
                      ...
                      children: [
                        Row(
                          ...
                          children: [
                            ...
                            InkWell(
                              onTap: () async {
                                await sharingThePostThroughtTheMessageOfTheApp(
                                    "${e.id ?? 0}", context);
                              },
                              child: CustomText(
                                Get
        ...
        builder: (BuildContext context) {
          return SingleChildScrollView(
            child: Padding(
              padding: const EdgeInsets.all(8.0),
              child: Column(
                children: peopleSareList!.map<Widget>((e) {
                  return SizedBox(
                    width: Get.width,
  ...
  //comments------------------------------------------------------------
  showingOtherComments(){
    showingAnotherComments = !showingAnotherComments;
    setState(() {
  ...
  addComment(BuildContext context) async {
    if(commentTextController.text.trim().isNotEmpty&&commentTextController.text != "Write Your Comment"){
      setState(() {
  ...
  getPostData() async {
    postData = await PostingServices.getPost(widget.postId);
    if(Get
        .find<StorageService>()
        .getUserType=="TEACHER") {
      teacherData = await TeacherServices.getTeacherInfoData();
    }else{
      personData = await PostingServices.getPersonInfoData();
    }
    if(Get
        .find<StorageService>()
        .getUserType=="TEACHER"){
      peopleSareList = await PostingServices.getTeacherShareList("${teacherData?.school??0}");
    }else if(Get
        .find<StorageService>()
        .getUserType=="STUDENT"){
      peopleSareList = await PostingServices.getStudentShareList("${personData?.school??0}", "${personData?.personInfoModelClass??0}");

    }else{
      parentSharModel = await PostingServices.getParentShareList("${personData?.school??0}", "${personData?.personInfoModelClass??0}");
  ...
  Widget build(BuildContext context) {
    return  Scaffold(
      ...
      const PostLoadingScreenChat():Container(
        width: Get.width,
        decoration: const BoxDecoration(
          color: kYellowColor,
        ),
              ...
              child: Center(
                child:  Column(
                  ...
                  children: [
                    ...
                    showingAnotherComments||((comments?.length??0)<3)?
                    Column(
                    ...
                    InkWell(
                      ...
                      child: SizedBox(
                        ...
                        child: CustomText(
                          textAlign: TextAlign.center,
                          maxLines: 1,
                          showingAnotherComments?Get.find<StorageService>()
                              .activeLocale ==
                              SupportedLocales.english
                              ? "Hide other comments"
                              :"أخفاء التعليقات الأخرى":Get.find<StorageService>()
                              .activeLocale ==
                              SupportedLocales.english
                              ? "Show other comments"
                                  ...
                                  child: TextField(
                                      ...
                                      decoration:   InputDecoration(
                                          focusedBorder: const OutlineInputBorder(
                                            ...
                                            borderSide: BorderSide(width: 2,color:kDarkBlueColor),
                                          ),
